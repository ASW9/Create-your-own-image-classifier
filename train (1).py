...
#this was suggested by mentor help as I was running into the following error:
#RuntimeError: size mismatch, m1: [64 x 9216], m2: [25088 x 2048]
#vgg19 uses 25088 for input layer, alexnet/densenet/whatever else doesn't as its a different arch type *note this for self use too*
...
if args.architecture == "vgg19":
    ...
    #was getting an issue here, after contacting mentor help I was suggested:
    #model =  getattr(models,arch)(pretrained=True)
    #in_features = model.classifier[0].in_features
    #however, the issue is that this gave me further errors, so I read the help given carefully and it was stated:
    # "Here the problem is you are using fixed sized feature layer for VGG19 model ('input_size': 25088) to load densenet model which has different size feature layer"
    # So I decided to change up my densenet model to ensure the feature layer was the same size as densenet121 and it worked!
    #note to self: do try understand the use of model =  getattr(models,arch)(pretrained=True) and in_features = model.classifier[0].in_features in spare time
    classifier = nn.Sequential(OrderedDict([
                              ('fc1', nn.Linear(1024, hu)),
                              ('relu', nn.ReLU()),
                              ('dropout', nn.Dropout(0.2)),
                              ('fc2', nn.Linear(hu, 256)),
                              ('relu', nn.ReLU()),
                              ('dropout', nn.Dropout(0.2)),
                              ('fc3', nn.Linear(256, 102)),
                              ('output', nn.LogSoftmax(dim=1))
